"""
ONEGEOLOGY_AGENT - Data Gateway Agent for Terra Constellata

Specialized agent for accessing Geology data from OneGeology Consortium.
"""
...
class Onegeology(DataGatewayAgent):
    """
    ONEGEOLOGY_AGENT data gateway agent.

    Provides access to Geology data from OneGeology Consortium.

    Capabilities: discover_geological_maps_by_nation, query_federated_geology_data
    """

    def __init__(self, llm, tools: List = None, **kwargs):
        """
        Initialize the ONEGEOLOGY_AGENT agent.
        ...
        default_config = {
            "base_url": "https://www.onegeology.org/",
            "api_key": "{{SECRETS.ONEGEOLOGY_AGENT_API_KEY}}",
        ...
        super().__init__(
            agent_name="ONEGEOLOGY_AGENT",
            data_domain="Geology",
            data_set_owner={'name': 'OneGeology Consortium', 'ownerType': 'Non-Profit', 'officialContactUri': 'https://www.onegeology.org/'},
            llm=llm,
            tools=tools or self._get_default_tools(),
            **config_merged
        )

        logger.info(f"Initialized ONEGEOLOGY_AGENT for Geology data access")
    ...
    async def _execute_capability(self, capability: str, **kwargs) -> Any:
        """
        Execute a specific capability against the OneGeology Consortium API.
