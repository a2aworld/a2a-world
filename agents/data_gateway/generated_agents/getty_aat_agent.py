"""
GETTY_AAT_AGENT - Data Gateway Agent for Terra Constellata

Specialized agent for accessing Thesaurus data from Getty Research Institute.
"""
...
class GettyAat(DataGatewayAgent):
    """
    GETTY_AAT_AGENT data gateway agent.

    Provides access to Thesaurus data from Getty Research Institute.

    Capabilities: get_term_by_id, search_terms
    """

    def __init__(self, llm, tools: List = None, **kwargs):
        """
        Initialize the GETTY_AAT_AGENT agent.
        ...
        default_config = {
            "base_url": "https://www.getty.edu/research/tools/vocabularies/aat/",
            "api_key": "{{SECRETS.GETTY_AAT_AGENT_API_KEY}}",
        ...
        super().__init__(
            agent_name="GETTY_AAT_AGENT",
            data_domain="Thesaurus",
            data_set_owner={'name': 'Getty Research Institute', 'ownerType': 'Non-Profit', 'officialContactUri': 'https://www.getty.edu/research/tools/vocabularies/aat/'},
            llm=llm,
            tools=tools or self._get_default_tools(),
            **config_merged
        )

        logger.info(f"Initialized GETTY_AAT_AGENT for Thesaurus data access")
