"""
PALI_CANON_AGENT - Data Gateway Agent for Terra Constellata

Specialized agent for accessing Religious Texts data from Vipassana Research Institute.
"""
...
class PaliCanon(DataGatewayAgent):
    """
    PALI_CANON_AGENT data gateway agent.

    Provides access to Religious Texts data from Vipassana Research Institute.

    Capabilities: get_sutta_by_id
    """

    def __init__(self, llm, tools: List = None, **kwargs):
        """
        Initialize the PALI_CANON_AGENT agent.
        ...
        default_config = {
            "base_url": "https://www.tipitaka.org/",
            "api_key": "{{SECRETS.PALI_CANON_AGENT_API_KEY}}",
        ...
        super().__init__(
            agent_name="PALI_CANON_AGENT",
            data_domain="Religious Texts",
            data_set_owner={'name': 'Vipassana Research Institute', 'ownerType': 'Non-Profit', 'officialContactUri': 'https://www.tipitaka.org/'},
            llm=llm,
            tools=tools or self._get_default_tools(),
            **config_merged
        )

        logger.info(f"Initialized PALI_CANON_AGENT for Religious Texts data access")
    ...
    async def _execute_capability(self, capability: str, **kwargs) -> Any:
        """
        Execute a specific capability against the Vipassana Research Institute API.
