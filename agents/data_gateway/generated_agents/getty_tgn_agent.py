"""
GETTY_TGN_AGENT - Data Gateway Agent for Terra Constellata

Specialized agent for accessing Gazetteer data from Getty Research Institute.
"""
...
class GettyTgn(DataGatewayAgent):
    """
    GETTY_TGN_AGENT data gateway agent.

    Provides access to Gazetteer data from Getty Research Institute.

    Capabilities: get_place_by_id, search_historical_places
    """

    def __init__(self, llm, tools: List = None, **kwargs):
        """
        Initialize the GETTY_TGN_AGENT agent.
        ...
        default_config = {
            "base_url": "https://www.getty.edu/research/tools/vocabularies/tgn/",
            "api_key": "{{SECRETS.GETTY_TGN_AGENT_API_KEY}}",
        ...
        super().__init__(
            agent_name="GETTY_TGN_AGENT",
            data_domain="Gazetteer",
            data_set_owner={'name': 'Getty Research Institute', 'ownerType': 'Non-Profit', 'officialContactUri': 'https://www.getty.edu/research/tools/vocabularies/tgn/'},
            llm=llm,
            tools=tools or self._get_default_tools(),
            **config_merged
        )

        logger.info(f"Initialized GETTY_TGN_AGENT for Gazetteer data access")
