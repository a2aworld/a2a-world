"""
NOAA_PALEOCLIMATE_AGENT - Data Gateway Agent for Terra Constellata

Specialized agent for accessing Climatology data from NOAA NCEI.
"""
...
class NoaaPaleoclimate(DataGatewayAgent):
    """
    NOAA_PALEOCLIMATE_AGENT data gateway agent.

    Provides access to Climatology data from NOAA NCEI.

    Capabilities: get_paleo_proxy_data
    """

    def __init__(self, llm, tools: List = None, **kwargs):
        """
        Initialize the NOAA_PALEOCLIMATE_AGENT agent.
        ...
        default_config = {
            "base_url": "https://www.ncdc.noaa.gov/paleo-search/",
            "api_key": "{{SECRETS.NOAA_PALEOCLIMATE_AGENT_API_KEY}}",
        ...
        super().__init__(
            agent_name="NOAA_PALEOCLIMATE_AGENT",
            data_domain="Climatology",
            data_set_owner={'name': 'NOAA NCEI', 'ownerType': 'Government Agency', 'officialContactUri': 'https://www.ncdc.noaa.gov/paleo-search/'},
            llm=llm,
            tools=tools or self._get_default_tools(),
            **config_merged
        )

        logger.info(f"Initialized NOAA_PALEOCLIMATE_AGENT for Climatology data access")
    ...
    async def _execute_capability(self, capability: str, **kwargs) -> Any:
        """
        Execute a specific capability against the NOAA NCEI API.
