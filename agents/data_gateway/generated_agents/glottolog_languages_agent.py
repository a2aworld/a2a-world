"""
GLOTTOLOG_LANGUAGES_AGENT - Data Gateway Agent for Terra Constellata
...
class GlottologLanguages(DataGatewayAgent):
    """
    GLOTTOLOG_LANGUAGES_AGENT data gateway agent.

    Provides access to Linguistics data from Max Planck Institute for Evolutionary Anthropology.

    Capabilities: get_language_by_glottocode, get_language_family_tree
    """

    def __init__(self, llm, tools: List = None, **kwargs):
        """
        Initialize the GLOTTOLOG_LANGUAGES_AGENT agent.
        ...
        default_config = {
            ...
            "api_key": "{{SECRETS.GLOTTOLOG_LANGUAGES_AGENT_API_KEY}}",
        ...
        super().__init__(
            agent_name="GLOTTOLOG_LANGUAGES_AGENT",
            data_domain="Linguistics",
            data_set_owner={'name': 'Max Planck Institute for Evolutionary Anthropology', 'ownerType': 'Academic Institution', 'officialContactUri': 'https://glottolog.org/'},
            llm=llm,
            tools=tools or self._get_default_tools(),
            **config_merged
        )

        logger.info(f"Initialized GLOTTOLOG_LANGUAGES_AGENT for Linguistics data access")
