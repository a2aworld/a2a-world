"""
USGS_SEISMIC_AGENT - Data Gateway Agent for Terra Constellata
...
class UsgsSeismic(DataGatewayAgent):
    """
    USGS_SEISMIC_AGENT data gateway agent.

    Provides access to Geospatial data from U.S. Geological Survey.

    Capabilities: get_earthquakes_by_region, get_realtime_seismic_feed
    """

    def __init__(self, llm, tools: List = None, **kwargs):
        """
        Initialize the USGS_SEISMIC_AGENT agent.
        ...
        default_config = {
            ...
            "api_key": "{{SECRETS.USGS_SEISMIC_AGENT_API_KEY}}",
        ...
        super().__init__(
            agent_name="USGS_SEISMIC_AGENT",
            data_domain="Geospatial",
            data_set_owner={'name': 'U.S. Geological Survey', 'ownerType': 'Government Agency', 'officialContactUri': 'https://earthquake.usgs.gov/'},
            llm=llm,
            tools=tools or self._get_default_tools(),
            **config_merged
        )

        logger.info(f"Initialized USGS_SEISMIC_AGENT for Geospatial data access")
