"""
PROJECT_GUTENBERG_AGENT - Data Gateway Agent for Terra Constellata

Specialized agent for accessing Literature data from Project Gutenberg.
"""
...
class ProjectGutenberg(DataGatewayAgent):
    """
    PROJECT_GUTENBERG_AGENT data gateway agent.

    Provides access to Literature data from Project Gutenberg.

    Capabilities: get_book_text_by_id, search_books_by_author
    """

    def __init__(self, llm, tools: List = None, **kwargs):
        """
        Initialize the PROJECT_GUTENBERG_AGENT agent.
        ...
        default_config = {
            ...
            "api_key": "{{SECRETS.PROJECT_GUTENBERG_AGENT_API_KEY}}",
        ...
        super().__init__(
            agent_name="PROJECT_GUTENBERG_AGENT",
            data_domain="Literature",
            data_set_owner={'name': 'Project Gutenberg', 'ownerType': 'Non-Profit', 'officialContactUri': 'https://www.gutenberg.org/'},
            llm=llm,
            tools=tools or self._get_default_tools(),
            **config_merged
        )

        logger.info(f"Initialized PROJECT_GUTENBERG_AGENT for Literature data access")
    ...
    async def _execute_capability(self, capability: str, **kwargs) -> Any:
        """
        Execute a specific capability against the Project Gutenberg API.
