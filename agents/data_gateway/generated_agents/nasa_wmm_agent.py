"""
NASA_WMM_AGENT - Data Gateway Agent for Terra Constellata

Specialized agent for accessing Geophysics data from NASA/NCEI.
"""
...
class NasaWmm(DataGatewayAgent):
    """
    NASA_WMM_AGENT data gateway agent.

    Provides access to Geophysics data from NASA/NCEI.

    Capabilities: get_magnetic_field_at_point
    """

    def __init__(self, llm, tools: List = None, **kwargs):
        """
        Initialize the NASA_WMM_AGENT agent.
        ...
        default_config = {
            "base_url": "https://www.ngdc.noaa.gov/geomag/WMM/",
            "api_key": "{{SECRETS.NASA_WMM_AGENT_API_KEY}}",
        ...
        super().__init__(
            agent_name="NASA_WMM_AGENT",
            data_domain="Geophysics",
            data_set_owner={'name': 'NASA/NCEI', 'ownerType': 'Government Agency', 'officialContactUri': 'https://www.ngdc.noaa.gov/geomag/WMM/'},
            llm=llm,
            tools=tools or self._get_default_tools(),
            **config_merged
        )

        logger.info(f"Initialized NASA_WMM_AGENT for Geophysics data access")
    ...
    async def _execute_capability(self, capability: str, **kwargs) -> Any:
        """
        Execute a specific capability against the NASA/NCEI API.
