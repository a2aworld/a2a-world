"""
METMUSEUM_ART_AGENT - Data Gateway Agent for Terra Constellata

Specialized agent for accessing Art History data from Metropolitan Museum of Art.
"""
...
class MetmuseumArt(DataGatewayAgent):
    """
    METMUSEUM_ART_AGENT data gateway agent.

    Provides access to Art History data from Metropolitan Museum of Art.

    Capabilities: get_object_by_id, search_openaccess_art
    """

    def __init__(self, llm, tools: List = None, **kwargs):
        """
        Initialize the METMUSEUM_ART_AGENT agent.
        ...
        default_config = {
            ...
            "api_key": "{{SECRETS.METMUSEUM_ART_AGENT_API_KEY}}",
        ...
        super().__init__(
            agent_name="METMUSEUM_ART_AGENT",
            data_domain="Art History",
            data_set_owner={'name': 'Metropolitan Museum of Art', 'ownerType': 'Non-Profit', 'officialContactUri': 'https://www.metmuseum.org/'},
            llm=llm,
            tools=tools or self._get_default_tools(),
            **config_merged
        )

        logger.info(f"Initialized METMUSEUM_ART_AGENT for Art History data access")
    ...
    async def _execute_capability(self, capability: str, **kwargs) -> Any:
        """
        Execute a specific capability against the Metropolitan Museum of Art API.
