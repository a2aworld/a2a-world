"""
ICONCLASS_SYMBOLS_AGENT - Data Gateway Agent for Terra Constellata

Specialized agent for accessing Iconography data from Iconclass Foundation.
"""
...
class IconclassSymbols(DataGatewayAgent):
    """
    ICONCLASS_SYMBOLS_AGENT data gateway agent.

    Provides access to Iconography data from Iconclass Foundation.

    Capabilities: get_classification_by_code, search_by_keyword
    """

    def __init__(self, llm, tools: List = None, **kwargs):
        """
        Initialize the ICONCLASS_SYMBOLS_AGENT agent.
        ...
        default_config = {
            "base_url": "https://www.iconclass.org/",
            "api_key": "{{SECRETS.ICONCLASS_SYMBOLS_AGENT_API_KEY}}",
        ...
        super().__init__(
            agent_name="ICONCLASS_SYMBOLS_AGENT",
            data_domain="Iconography",
            data_set_owner={'name': 'Iconclass Foundation', 'ownerType': 'Non-Profit', 'officialContactUri': 'https://www.iconclass.org/'},
            llm=llm,
            tools=tools or self._get_default_tools(),
            **config_merged
        )

        logger.info(f"Initialized ICONCLASS_SYMBOLS_AGENT for Iconography data access")
    ...
    async def _execute_capability(self, capability: str, **kwargs) -> Any:
        """
        Execute a specific capability against the Iconclass Foundation API.
