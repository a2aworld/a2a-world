"""
OPENSTREETMAP_AGENT - Data Gateway Agent for Terra Constellata

Specialized agent for accessing Geospatial data from OpenStreetMap Foundation.
"""
...
class Openstreetmap(DataGatewayAgent):
    """
    OPENSTREETMAP_AGENT data gateway agent.

    Provides access to Geospatial data from OpenStreetMap Foundation.

    Capabilities: get_vector_data_by_bbox, query_road_network
    """

    def __init__(self, llm, tools: List = None, **kwargs):
        """
        Initialize the OPENSTREETMAP_AGENT agent.
        ...
        default_config = {
            ...
            "api_key": "{{SECRETS.OPENSTREETMAP_AGENT_API_KEY}}",
        ...
        super().__init__(
            agent_name="OPENSTREETMAP_AGENT",
            data_domain="Geospatial",
            data_set_owner={'name': 'OpenStreetMap Foundation', 'ownerType': 'Community', 'officialContactUri': 'https://www.openstreetmap.org/'},
            llm=llm,
            tools=tools or self._get_default_tools(),
            **config_merged
        )

        logger.info(f"Initialized OPENSTREETMAP_AGENT for Geospatial data access")
    ...
    async def _execute_capability(self, capability: str, **kwargs) -> Any:
        """
        Execute a specific capability against the OpenStreetMap Foundation API.
